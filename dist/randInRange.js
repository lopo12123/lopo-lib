 ...
 * @param step 步长
 */
const randWithStep = (left, right, exclude = 'right', step) => {
    let p = Math.random() * (right - left) + left;
    while (exclude === 'left' && p === left)
        p = Math.random() * (right - left) + left;
    if (!step)
        return p;
    else {
        let above = p % step;
 ...
 */
const randInRange = (left, right, exclude = 'right', step = 0) => {
    if (right <= left)
        throw new Error('right <= left is not allowed.');
    else if (step !== undefined && right - left <= step)
        throw new Error('step <= right - left is not allowed.');
    else {
        if (exclude === 'right')
            return randWithStep(left, right, 'right', step);
        else if (exclude === 'left')
            return randWithStep(left, right, 'left', step);
        else {
