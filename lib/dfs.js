...
const _deep = (childTree, condition, childKey, resultFilter) => {
    ...
    if(condition(childTree)) {
        // (TODO: never) if [resultFilter] is [undefined] - directly return
 ...
 * @return {Object|null}
 */
const dfs = (tree, condition, childKey, resultFilter) => {
    childKey = childKey ? childKey : 'children'
    resultFilter = resultFilter ? resultFilter : (node) => { node[childKey] = []; return node }
